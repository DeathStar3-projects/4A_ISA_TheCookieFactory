# Business Components
  ...
  * Prev.: [Architecture](https://github.com/collet/4A_ISA_TheCookieFactory/blob/develop/chapters/Architecture.md)
...
__Warning__: When a component implements an interface, it is supposed to properly implement the contract. Thus, if the container does not find a bean with the given name, it will chose one randomly among the available one, as it should be semantically equivalent.
  ...
  * Next: [Exposing components as Web Services (SOAP)](https://github.com/collet/4A_ISA_TheCookieFactory/blob/develop/chapters/Exposing_SOAP.md)collet